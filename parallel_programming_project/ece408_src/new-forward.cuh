...
__global__ void matrixMultiplyShared(float * __restrict__ A, float * __restrict__ B, float * __restrict__ C, int M, int C_len, int H, int W, int K) //  int numAColumns, int numCRows, int numCColumns, int K, int W_out, int H, int W)
{
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w)
{
  ...
  dim3 dimgrid(ceil((float) (H*W) / TILE_WIDTH), ceil((float) M / TILE_WIDTH));
  dim3 dimblock(TILE_WIDTH, TILE_WIDTH);

  for (int batch = B-1; batch>=0; batch--) {
    matrixMultiplyShared<<<dimgrid, dimblock>>>(w.dptr_, ((C*H*W*batch) + x.dptr_), batch*(H_out*W_out)*M + (y.dptr_), M, C, H, W, K);
  }
