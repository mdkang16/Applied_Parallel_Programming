...
namespace op
{
__constant__ float k[10000]; //7200 is enough for forward2
    ...
    */
	...
	for (int y_mask = 0; y_mask < K; y_mask++){
		...
		for (int x_mask = 0; x_mask < K; x_mask++){
		  ...
		  res += k4d(m, c, y_mask, x_mask) * xTile(threadIdx.y+y_mask, threadIdx.x+x_mask);
		}
	}
	__syncthreads();
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w)
{
    ...
    */
    ...
    int xTileWidth = TILE_WIDTH + K-1;
    dim3 gridDim(B, M*C, Z);
    dim3 blockDim(TILE_WIDTH, TILE_WIDTH, 1);
    // Allocate shared memory space in runtime
    size_t shmem_size = (xTileWidth*xTileWidth)*sizeof(float);
    // Copy constant memory
    cudaMemcpyToSymbol(k, w.dptr_, sizeof(float)*M*C*K*K);
