...
__global__ void matrixMultiplyShared(float *A, float *B, float *C, int numAColumns, int numCRows, int numCColumns)
{
  ...
  for(int i = 0; i < ceil((1.0) * numAColumns/TILE_WIDTH); i++) {
    mytileM[ty][tx] = A[row * numAColumns + i * TILE_WIDTH + tx];
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w)
{
    ...
    for (int batch = B-1; batch>=0; batch--) {
        ...
        matrixMultiplyShared<<<dimgrid, dimblock>>>(w.dptr_, op_unroll, batch * (H_out * W_out) * M + (y.dptr_), K * K * C,  M,  H_out * W_out);
    }
    cudaFree(op_unroll);
